...
export default class Camera
{
    ...
    setInstance()
    {
        ...
        this.time.on('tick', () =>
        {
            if(!this.orbitControls.enabled)
            {
                ...
                // A decaying camera-space impulse, without moving the HUD or canvas.
                if (this.impactShake > 0.002) {
                    const phase = this.time.elapsed * 0.075
                    this.instance.position.x += Math.sin(phase) * this.impactShake
                    this.instance.position.y += Math.cos(phase * 1.3) * this.impactShake
                    this.impactShake *= Math.exp(-this.time.delta / 95)
                }
            }
        })
    }
    ...
    setZoom()
    {
        ...
        this.zoom.value = this.config.cyberTruck ? 0.15 : 0.2
