import { floorLabel, labelTexture } from '../../portfolio/sceneText.js'
...
export default class IntroSection
{
    ...
    setBillboard()
    {
        ...
        const by = 3
        const rotationZ = 0
        this.billboard.color = new THREE.Color('#ffffff')

        // Moldura com colisão (reaproveita a estrutura das placas de projeto)
        this.objects.add({
            ...
            offset: new THREE.Vector3(bx, by, 0),
            rotation: new THREE.Euler(0, 0, rotationZ),
            duplicated: true,
            mass: 0
        })

        // Grupo que mantém tela e moldura alinhadas e giram juntas
        const group = new THREE.Object3D()
        group.position.set(bx, by, 0)
        group.rotation.z = rotationZ
        this.container.add(group)

        // Tela do banner: mantém a transformação nativa do plano (rotação/escala que casam com a moldura)
        const planeSource = this.resources.items.projectsBoardPlane.scene.children[0]
        const plane = planeSource.clone()
        plane.material = new ProjectBoardMaterial()
        plane.material.side = THREE.DoubleSide
        plane.material.uniforms.uColor.value = this.billboard.color
        plane.material.uniforms.uTextureAlpha.value = 0
        group.add(plane)
        this.billboard.plane = plane
        ...
        image.addEventListener('load', () =>
        {
            ...
            gsap.to(plane.material.uniforms.uTextureAlpha, { value: 1, duration: 1.5, ease: 'power4.inOut' })
        })
        image.src = './banners/allans-and-dogs.jpg'
    }
    ...
    setInstructions()
    {
        ...
        this.instructions.arrows.label.texture = labelTexture(['USE AS SETAS OU WASD'])
