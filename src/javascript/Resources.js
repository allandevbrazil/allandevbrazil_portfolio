...
export default class Resources extends EventEmitter
{
    constructor()
    {
        ...
        this.loader.load([
            ...
            { name: 'matcapNeonPink', source: './models/matcaps/neonPink.png', type: 'texture' },
        ...
        ].filter(resource => !/^(intro(?:[BRUNOSIM](?:Base|Collision)|Static)|introCreative|introDev|information|crossroads|projects(?!Board)|distinction|konami|wig)/.test(resource.name)))
        ...
        this.loader.on('end', () =>
        {
            for(const [name, text] of Object.entries({areaOpenTexture:'ABRIR', areaEnterTexture:'ENTRAR', areaResetTexture:'REINICIAR'})) this.items[name] = labelTexture([text])
            // Trigger ready
            this.trigger('ready')
        })
    }
